;(function () {
    ...
    function render_task_list() {
        ...
        for(var i = 0;i<task_list.length;i++){
            ...
            if( item && item.complete)//如果这条item被选中 那么就把它添加到已完成的那一类中
                complete_items[i] = item;
                // complete_items.push();//把数据存入到complete_items中
            else
        ...
        for(var j = 0;j < complete_items.length;j++){
            ...
            if(!$task) continue;
            $task.addClass('completed');//所有选中复选框已完成的item添加一个类名completed
            $task_list.append($task);//把复选框选中的item添加到整个列表的最后
        }

        $task_delete_trigger = $('.action.delete');//trigger是触发的意思
        $task_detail_trigger = $('.action.detail');
        $checkbox_complete = $('.task-list .complete[type=checkbox]');//每条item的复选框
...
//****************************************************************************************************
    ...
    function Listen_task_detail() {
        var index;
        //选中所有的task-item 即选中所有的任务列表 只要双击列表就直接触发
        $('.task-item').on('dblclick',function () {
            index = $(this).data('index');//$(this)表示当前点击的这条item  获取它的index
            show_task_detail(index);//调用显示详情
        });
        //点击item中的详情按钮 触发下面的事件
        $task_detail_trigger.on('click',function () {
            var $this = $(this);//当前的item的详情按钮
            // console.log('$this',$this.parent())
            var $item = $this.parent().parent();//$this.parent()表示所选的详情按钮的父元素,class=fr的span标签
            //$item表示所选中的详情按钮的父元素的父元素  即这一条item
    ...
    function update_task(index,data) {
        ...
        //jquery中的merge()是用于合并数组 ，extend()用于合并对象
        refresh_task_list();
    }
...
//****************************************************************************************
    ...
    function Listen_checkbox_complete() {
        $checkbox_complete.on('click',function () {
            ...
            // console.log('item',item)
            if(item.complete)
                //complete属性是用来检测浏览器是否完成加载  加载完成返回true，失败返回false
    ...
    function get(index) {
        ...
        //store.get('task_list')表示的是所有储存的item以及其全部信息  [index]只是确定到具体的一条
    }
//****************************************************************************************
    function pop(arg) {
        if(!arg){
